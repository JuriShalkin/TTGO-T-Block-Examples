...
void loop() {
  if (millis() - runTime >= 1000L) {                                            // Execute every 2s
    ...
    ringMeter(reading, 0, 200, 10, 10, 110, "Watts", BLUE2RED);                 // Draw analogue meter
  }
}
...
int ringMeter(int value, int vmin, int vmax, int x, int y, int r, char *units, byte scheme) {
  ...
  tft.setFreeFont(FF7);                 // Select the font

  // Print value, if the meter is large then use big font 6, othewise use 4
  if (r > 84) {
    tft.setTextSize(2); //2 * text size, default text size: 6 * 8
    tft.setCursor(x - 30, y - 20); //set text position to center
    //tft.print(buf);
    tft.drawCentreString(buf, x - 5, y - 20, GFXFF); // Value in middle
  }
  else {
    tft.setTextSize(2); //2 * text size, default text size: 6 * 8
  ...
  if (r > 84) {
    tft.setTextSize(2); //2 * text size, default text size: 6 * 8
    tft.setCursor(x - 30, y + 10); //set text position to center
    tft.print(units);
